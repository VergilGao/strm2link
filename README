...
详情请参考这个 [issue]('https://github.com/jellyfin/jellyfin/issues/16557')
...
- 当 `.strm` 文件被 **创建、修改** 时：
  - 读取文件内容（目标媒体文件的绝对路径）。
  - 在 `STRM_LINK_DIR` 下创建相同相对目录结构，生成指向该绝对路径的符号链接，链接名为目标文件的文件名。
- 当 `.strm` 文件被 **删除** 时：
  - 删除对应的符号链接。
...
- `.strm` 文件内容必须是**绝对路径**（如 `/data/media/movie.mp4` 或 `C:\data\media\movie.mp4`）。
- 生成的符号链接指向的目标文件路径**必须在读取该链接的容器或系统中存在**，否则链接断裂。
